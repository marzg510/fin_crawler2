...
def input_secret_word():
    ...
    e_q = driver.find_element(By.XPATH, '//dt[text()="質問"]/following-sibling::dd')
...
try:
    ...
    e_submit = driver.find_element(By.XPATH, '//input[@type="button" and @name="U010103"]')
    ...
    e_password = driver.find_element(By.NAME, 'loginPassword')
    e_login = driver.find_element(By.NAME, 'U010302')
    ...
    if 'ゆうちょ銀行からのお知らせ' in driver.page_source:
        ...
        e_next = driver.find_element(By.XPATH, '//a[text()="ダイレクトトップ"]')
        e_next.click()
        helper.ss(name='yuucho-direct-top-after-confirm')

    # ############ Navigate to Detail page
    log.info("Navigate to Detail page")
    e_link = driver.find_element(By.XPATH, '//a[text()="入出金明細照会"]')
    ...
    e_term = driver.find_element(By.XPATH, '//label[contains(text(),"直近６か月分")]')
    log.debug('term radio : %s %s visible=%s' % (e_term.tag_name, e_term.text, e_term.is_displayed()))
    e_term.click()
    helper.ss(name='after-click-term')
    e_csv_link = driver.find_element(By.XPATH, '//a[@onclick and text()="データダウンロード（CSV形式）"]')
    ...
    e_csv = driver.find_element(By.XPATH, '//a[text()="ダウンロード"]')
